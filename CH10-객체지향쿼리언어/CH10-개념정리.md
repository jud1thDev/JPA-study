...
앞서 언급했듯, JPQL 빌더 역할을 하는 Criteria는 컴파일 타임 타입 체크와 자동완성 기능 덕분에 유용하지만 코드가 너무 복잡해서 권장되지 않는다. 
이걸 대체할 수 있는 게 QueryDSL이다. QueryDSL도 코드 기반으로 JPQL을 만들 수 있는 빌더 역할을 하지만, 훨씬 직관적이고 간결하다.
QueryDSL이라는 이름 그대로, 쿼리를 조회하는 데 기능이 특화되어 있다.
...
QueryDSL을 사용하려면 먼저 Maven 설정을 마친 뒤 명령어 mvn compile을 실행하면, target/generated-sources 디렉토리에 Q엔티티명 형식의 쿼리 타입 클래스들이 생성된다. (Member 엔티티가 있으면 QMember라는 클래스)
이 Q클래스를 통해 코드로 안전하고 편리하게 JPQL을 작성할 수 있다.
...
1. QueryDSL을 사용하려면 우선 JPAQuery 객체를 생성함: JPAQuery 객체는 EntityManager를 받아 생성되며, 쿼리를 시작할 수 있게 해주는 객체
2. 사용할 쿼리 타입 생성, 생성자에게는 별칭 지정해야 함: QMember는 Member 엔티티에 대한 쿼리 타입 클래스이며, 별칭은 "m"
3. from(), where(), orderBy() 메서드로 쿼리를 코드로 간단하게 표현
...
- 이렇게 생성된 QAccount.account처럼 static 필드로 제공되기 때문에 바로 사용할 수 있다. 
- 하지만 같은 엔티티를 조인하거나 서브쿼리에 사용하면 별칭이 겹칠 수 있다. 이럴 때는 new QAccount("다른이름")처럼 직접 별칭을 지정해서 생성해야 한다.

<br>

## 프로젝션과 결과 반환

- 프로젝션: select 절에 어떤 값을 조회할지를 지정하는 것
...
```
- 하나의 필드만 조회하면 그 필드의 타입으로 결과가 바로 반환된다. 
- 위 예시에서는 item.name을 조회했으므로 List<String>이 반환된다. 
...
```
- 프로젝션 대상으로 여러 필드를 선택하면 QueryDSL은 기본적으로 Tuple이라는 결과 타입을 사용한다. Tuple은 여러 필드 값을 담을 수 있는 QueryDSL 전용 타입이다.
- tuple.get(쿼리타입필드) 방식으로 원하는 값을 꺼낼 수 있다.
- 배열 대신 필드명을 통해 값을 읽을 수 있어 명확하다.

## 빈 생성 DTO

- 쿼리 결과를 엔티티가 아닌 별도의 DTO 객체로 받고 싶을 때는, QueryDSL이 제공하는 빈 생성 기능을 사용하면 된다. 이 방식은 실무에서도 자주 사용된다. 
- QueryDSL은 DTO 객체를 생성하는 다양한 방식을 지원한다: 이 방법들을 쓰기 위해 Projections 클래스를 활용
  - 프로퍼티 접근 (setter 이용)
  - 필드 직접 접근 
  - 생성자 사용
...
- 프로퍼티 접근: bean()은 DTO의 setter 메서드를 이용해서 값을 채운다.
- as("username")을 사용해 DTO 필드명과 쿼리 필드명을 맞춰준다.
...
- 필드 직접 접근: fields()는 DTO의 필드에 직접 값을 넣는다.
- 필드가 private여도 동작한다.
...
- 생성자 사용: 지정한 순서와 타입에 맞는 생성자가 DTO에 있어야 한다.
- 타입이 안 맞으면 컴파일 에러는 안 나지만 런타임 에러가 날 수 있다.

<br>

## 수정, 삭제 배치 쿼리

QueryDSL도 여러 건을 한꺼번에 수정하거나 삭제하는 배치 쿼리를 지원한다.
이때는 JPQL의 배치 쿼리처럼 영속성 컨텍스트를 무시하고 DB에 직접 쿼리가 실행된다는 점을 꼭 기억해야 한다.
...
- 조건에 맞는 데이터를 찾아 price를 100만큼 증가시킨다.
- 반환값은 영향을 받은 row 수이다.
...
BooleanBuilder를 사용하면, 입력값에 따라 조건을 붙이거나 빼는 동적 쿼리를 만들 수 있다. 복잡한 조건을 if문으로 유연하게 조립할 수 있어서 매우 유용하다.
...
- 조건이 null이면 where절에 포함되지 않고,
- 값이 있으면 자동으로 해당 조건이 붙는다.

<br>

## 메서드 위임

- Delegate Method: 자주 사용하는 조건은 별도의 메서드로 정의해두고 쿼리 타입에 위임할 수 있다.
...
- @QueryDelegate는 어떤 엔티티에 대한 조건식인지 지정한다. 
- 첫 번째 파라미터는 해당 엔티티의 Q타입이다.
...
- 위처럼 메서드가 Q타입에 생성되므로 쿼리에서 간편하게 사용할 수 있다.
...
- 자주 쓰는 조건을 재활용할 수 있어서 코드가 깔끔해진다.
...
JPA의 JPQL은 SQL처럼 다양한 기능을 지원하지만, 특정 데이터베이스에만 있는 기능은 지원하지 않는다.
예를 들어 오라클의 CONNECT BY, 특정 DB만 사용하는 힌트(HINT), UNION, INTERSECT, 스토어드 프로시저 같은 것들이다.
이런 기능이 꼭 필요할 때는 JPQL 대신 네이티브 SQL을 사용해야 한다.

<br>

## JPQL에서 특정 데이터베이스에 종속적인 기능을 지원하는 방법

- 특정 데이터베이스만 사용하는 함수  
  → JPA 2.1부터 네이티브 SQL 함수를 호출할 수 있다.

- 하이버네이트는 각 데이터베이스에 맞는 방언(Dialect)을 만들어서 전용 함수들을 미리 정의해둔다.

- 특정 DB만 지원하는 SQL 쿼리 힌트  
  → 하이버네이트를 포함한 일부 구현체에서 지원한다.

- 인라인 뷰, UNION, INTERSECT  
  → JPQL은 지원하지 않지만 일부 JPA 구현체에서는 네이티브 SQL로 가능하다.
...
JDBC에서도 SQL을 직접 사용할 수 있지만, JPA의 네이티브 SQL은 **영속성 컨텍스트와 함께 작동**한다.
즉, 네이티브 SQL로 데이터를 조회해도, JPA가 해당 데이터를 엔티티로 변환해주고 변경 감지 등도 정상적으로 동작할 수 있다.
...
```
- 엔티티 클래스 타입을 알려주면 결과를 자동으로 매핑해준다.
...
- 복잡한 결과 매핑이 필요할 때 resultSetMapping이라는 이름으로 별도 매핑을 지정해 사용할 수 있다.

### Named 쿼리

네이티브 SQL도 JPQL을 사용할 때와 마찬가지로 Named 쿼리를 사용할 수 있다.
네이티브 SQL을 @NamedNativeQuery로 등록하면 재사용이 가능하고 관리가 쉬워진다.
반환 타입이 명확하다면 TypedQuery를 그대로 사용할 수 있기 때문에 JPQL처럼 동작할 수 있다.
...
엔티티가 수백 개 이상일 때, 하나하나 `persist()` 하거나 `remove()` 하면 너무 느리다.  
이럴 땐 한 번에 여러 데이터를 처리하는 **벌크 연산(Bulk Operation)**을 사용하면 된다.

JPA는 다음과 같은 JPQL 문법으로 벌크 연산을 지원한다.
...
예를 들어, 재고 수량이 10보다 적은 상품의 가격을 10% 올릴 때는 아래와 같이 쓴다.
...
```java
em.createQuery(위 JPQL).setParameter("stockAmount", 10).executeUpdate();
```

### DELETE 벌크 연산

예를 들어, 가격이 100 미만인 상품을 삭제하고 싶을 땐 이렇게 쓴다.
...
```java
em.createQuery(위 JPQL).setParameter("price", 100).executeUpdate();
```

### INSERT 벌크 연산 (하이버네이트만 지원)

JPA 표준에는 없지만, 하이버네이트는 INSERT SELECT 방식으로 벌크 INSERT도 지원한다.

### 벌크 연산의 주의점

벌크 연산은 **영속성 컨텍스트를 무시**하고 DB에 바로 반영되기 때문에, 영속 상태의 엔티티와 DB 상태가 달라질 수 있다.

예를 들어:
1. 가격이 1000원인 상품 A를 조회함 → 이때 상품 A는 영속성 컨텍스트에 올라감
2. 벌크 UPDATE로 모든 상품의 가격을 10% 올림 (A의 가격도 1100원이 됨)
3. 하지만 상품 A는 여전히 메모리에 1000원 상태로 남아있음

→ 벌크 연산은 DB만 바뀌고, 메모리는 그대로인 상태가 되는 것이다.

### 해결 방법

- **em.refresh(상품A)** : DB 상태로 다시 불러옴
- **벌크 연산 먼저 실행 후 조회** : 이러면 아예 최신 DB 기준으로 조회됨
- **em.clear() 사용** : 영속성 컨텍스트 초기화

→ 보통 벌크 연산은 가장 먼저 실행하는 게 좋고, 이후 조회할 경우 `em.clear()` 해주는 게 안전하다.

<br>

## 10.6.2 find() vs JPQL

- em.find()는 먼저 1차 캐시(영속성 컨텍스트)를 확인하고, 없으면 DB를 조회한다.  
  → 그래서 성능이 빠르고, 동일한 엔티티가 있으면 기존 객체를 그대로 반환한다.

- 반면 JPQL은 항상 DB에 SQL을 날려서 결과를 가져온다.  
  → 1차 캐시를 무시하는 게 아니라, 캐시에 있는 엔티티와 동일한 식별자면 병합한다.

<br>

## 10.6.3 JPQL과 플러시 모드

플러시란, 영속성 컨텍스트에서 변경된 내용을 DB에 반영하는 작업이다.  
JPA는 트랜잭션 커밋 시나 JPQL 실행 전에 자동으로 플러시를 수행한다.

### 플러시 모드 종류

- 기본값: `FlushModeType.AUTO`  
  → 쿼리 실행 직전, 커밋 직전에 자동 플러시가 일어난다

- 설정값: `FlushModeType.COMMIT`  
  → 커밋할 때만 플러시가 일어나고, 쿼리 실행 전에는 일어나지 않는다

1. 상품의 가격을 1000 → 2000으로 수정함 (엔티티만 바뀐 상태)
2. 그 상태에서 `select p from Product p where p.price = 2000` JPQL을 실행함
3. FlushMode가 AUTO면 쿼리 실행 전 플러시가 자동으로 일어나서 DB 반영됨
4. FlushMode가 COMMIT이면 플러시가 안 일어나서, DB에는 아직 1000으로 남아있고 결과가 안 나올 수 있음

### 왜 COMMIT 모드를 쓰는가?

- 플러시가 너무 자주 발생해서 성능에 부담이 되는 경우
- 예: 대량의 조회 쿼리를 반복적으로 실행하는데 매번 플러시가 일어나는 상황

단점은 데이터 불일치 가능성인데, 그걸 감수하고도 성능을 얻고 싶을 때 선택한다.